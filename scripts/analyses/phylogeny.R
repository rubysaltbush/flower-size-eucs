...
# plot contMap
plot.contMap(treecont, type = "fan",
             ftype = "off", lwd = 3, outline = FALSE, sig = 1, #mar = c(5, 0.2, 0.2, 2),
...
# plot contMap
plot.contMap(treecont, sig = 1, type = "fan",
             ftype = "off", lwd = 2, outline = FALSE, mar = c(5, 0.2, 0.2, 2),
             leg.txt = "Bud size (log mm²)", add = TRUE)

# redrawing trick not working, might just have to add axis in gap
...
# legends
legend(x = "topleft", legend = c("white-cream", "mixed", "colourful"), col = fcols, 
       bty = "n", cex = 0.8, title = "Flower colour", pch = 15)

dev.off()
rm(pp, budsz_contdata, fcol_data, fcols, tree_budsz, treecont, contMap_data)
# to do - offset tip labels from circle
#       - fix time axis
# to do - clade labels? or label western/eastern Australia clades?
